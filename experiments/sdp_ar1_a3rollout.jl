...
const TAG    = get(ENV, "TAG", "a3_rollout")
...
@everywhere begin
    ...
    # ── A₃v2: One-step forecast rollout ───────────────────────────────────
    #    Keep A₂ value function unchanged. Replace the AR(1) mean prediction
    #    α·z + β in the stage cost with the real-time one-step-ahead forecast
    #    b_t = forecast_load[1] - forecast_pv[1].
    #    The value function V(soc', z') still assumes AR(1) dynamics = correct.
    #    This is a policy improvement step (rollout) — can only improve or equal.
    # ─────────────────────────────────────────────────────────────────────────
    function EMSx.compute_control(controller::SdpAr1A2, information::EMSx.Information)
        ...
        # One-step-ahead forecast (b_t = ẑ_{t,t+1})
        b_t = information.forecast_load[1] - information.forecast_pv[1]

        # Swap in forecast-aware cost temporarily
        orig_cost = controller.model.cost
        _bp = information.prices.buy
        _sp = information.prices.sell
        _pw = information.battery.power

        function rollout_cost(t::Int64, state::Array{Float64,1},
                              control::Array{Float64,1}, noise::Array{Float64,1})
            z_pred = b_t + noise[1]  # forecast baseline + AR(1) residual noise
            ckwh = control[1] * _pw * 0.25
            ie = ckwh + z_pred
            return (_bp[t] * max(0., ie) - _sp[t] * max(0., -ie))
        end

        controller.model.cost = rollout_cost
        ...
        controller.model.cost = orig_cost
        return control[1]
    end

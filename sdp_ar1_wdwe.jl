...
@everywhere begin
    ...
    function fit_linear_noise_model_wdwe(path_to_data_csv::String, k::Int64=K_NOISE)
        ...
        for t in 1:horizon
            ...
            # Compute residuals for this specific time slot
            xs_t = [t == 1 ? weeks_data[w, horizon] : weeks_data[w, t-1] for w in 1:n_weeks]
            ys_t = [weeks_data[w, t] for w in 1:n_weeks]
            X_t = hcat(xs_t, ones(length(xs_t)))
            epsilon = ys_t - X_t * w_reg

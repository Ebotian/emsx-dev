...
### 3.8 价值函数价格失配伪象 + 比值重训实验（probe_vf_ratio）
前期"电价倍数扫描"（峰价 ×{1..40} 但价值函数保持 1× 训练）曾得出"SE 在 ≥3× 反超、曲面 ≥3× 平坦"的结论。经 2D 曲面 + SE-vs-perfect 检查发现为**伪象**：价值函数按 1× 价格训练却部署在缩放价格下，其 z 梯度成为错误尺度的有害信号——40× 时完美预测（λ=1）反而比不完美的 SE 调度更差（铁证）。修正实验 `experiments/probe_vf_ratio.jl`（固定 AR(1) 预测，7 个训练比值 × 7 个部署比值重训价值函数，10 线程并行）给出干净结果（`experiments/data/probe_vf_ratio.json`，等高线图 `research/vf_ratio_contour.png`）：
- **匹配对角线**（训练=部署）：收益随峰谷比近似线性增长（每翻倍 ~2×，1× 8783 → 40× 1053805 €）
- **失配惩罚巨大且不对称**：1× 训练部署到 40× 仅剩 38% 收益（损失 62%）；40× 训练部署到 1× 仅剩 22%（损失 78%，过训练惩罚 > 欠训练）；相邻比值损失 11-15%
- **结论**：价值函数必须按部署电价（或近邻）训练；"价值函数族 + 运行点匹配选择 + OTA 重编译"不是锦上添花，而是避免 22-78% 收益损失的关键机制——直接验证"结构自适应可行最优自动编译调度控制器"架构的必要性
